...
def gaitscan_frontal():
  ...
  def gaitscan_frontal():
    ...
    def process_video_frontal(video_file, frame_skip=2):
        ...
        results = {joint: [] for joint in ["Hanche G","Genou G","Cheville G",
        ...
        while cap.isOpened():
            ...
            if frame_idx % frame_skip == 0:
                ...
                results["Cheville G"].append(angle(kp[JOINTS_IDX["Genou G"],:2],
                                                   kp[JOINTS_IDX["Cheville G"],:2],
                                                   kp[JOINTS_IDX["Cheville G"],:2]+np.array([1,0])))
                results["Cheville D"].append(angle(kp[JOINTS_IDX["Genou D"],:2],
                                                   kp[JOINTS_IDX["Cheville D"],:2],
                                                   kp[JOINTS_IDX["Cheville D"],:2]+np.array([1,0])))
                # Tronc (inclinaison latérale)
                results["Tronc"].append(angle(kp[JOINTS_IDX["Epaule G"],:2],
    ...
    if live_cam:
        ...
        if cam_file:
            video_file_to_use = cam_file
            video_ready = True
    elif video_file:
        video_file_to_use = video_file
        video_ready = True

    if video_ready and st.button("⚙️ Lancer l'analyse"):
        with st.spinner("Analyse en cours..."):
            tfile = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
            tfile.write(video_file_to_use.read())
            ...
            for (joint_pair, normal_func) in zip(articulation_pairs, normal_funcs):
                ...
                for joint, color in zip(joint_pair, ['red','blue']):
                    ...
                    summary_table.append([joint, f"{np.min(results[joint]):.1f}",
                                          f"{np.mean(results[joint]):.1f}", f"{np.max(results[joint]):.1f}"])
                ...
                if show_normal:
                    ...
                    normal_curve = normal_func(length, sigma=smoothing*2)
            ...
            summary_table.append(["Pelvis", f"{np.min(results['Pelvis']):.1f}",
                                  f"{np.mean(results['Pelvis']):.1f}", f"{np.max(results['Pelvis']):.1f}"])
            ...
            summary_table.append(["Tronc", f"{np.min(results['Tronc']):.1f}",
                                  f"{np.mean(results['Tronc']):.1f}", f"{np.max(results['Tronc']):.1f}"])
